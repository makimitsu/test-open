function [IDSPdata] = cal_ave_ICCD_ionflow(IDSP,pathname,show_offset,plot_fit,save_fit)
...
l_mov_temp = 5;%【input】波長方向移動平均長さ[px]15
th_ratio_temp = 0.8;%【input】フィッティング時閾値[px]0.8
l_L1 = 61;%【input】波長軸の切り取り長さ[px]
d_L1 = 38;%【input】波長方向位置ずれ調整[px]
d_CH = -7;%【input】CH方向位置ずれ調整[px]
pre_offset = 0.086;%【input】分光器波長方向位置ずれ調整[nm]

savename = [pathname.mat,'/ionflow/',num2str(IDSP.date),'_shot',num2str(IDSP.shotlist(1)),'-',num2str(IDSP.shotlist(end)),'_',num2str(IDSP.delay),'us_w=',num2str(IDSP.width),'_gain=',num2str(IDSP.gain),'.mat'];
if exist(savename,"file")
    ...
    cnt = 0;
    if IDSP.n_z == 1
        for i =1:size(IDSP.shotlist,1)
            filename1 = [dir1 '/shot' num2str(IDSP.shotlist(i)) '_' num2str(IDSP.delay) 'us_w=' num2str(IDSP.width) '_gain=' num2str(IDSP.gain) '.asc'];%ICCDファイル名
            if not(exist(filename1,"file"))
                ...
                if cnt == 0
                    ...
                    data1 = data1 + importdata(filename1);
                end
                cnt = cnt+1;
            end
        end
    ...
    offset = zeros(IDSP.n_r,IDSP.n_z);
    ...
    offset(idx_non0,1) = buf;%offsetを更新
    offset(idx_0,1) = mean(offset(idx_non0,1));%offsetを更新
    for k = 1:IDSP.n_r
    ...
    offset(5,1) = offset(6,1);%5番の視線1と視線2が連番でなくoffsetが信用できないため導入(20230807校正のとき)
    ...
    if plot_fit
        ...
        sgtitle(['Fitting data for flow (Horizontal：View Line, Vertical：Measured Position)',newline, ...
            'shot',num2str(IDSP.shotlist(1)),'-',num2str(IDSP.shotlist(end)),'-',num2str(IDSP.delay),'us-w=',num2str(IDSP.width),'-gain=',num2str(IDSP.gain),'.asc'])
    end
    for k = 1:IDSP.n_r
        ...
        for i = 1:4
            i_CH = (k-1)*4+i;%CH番号
            S1 = [L1_shaped(:,i_CH)-offset(k,1) spectrum1_flow(:,i_CH)]; %[波長,強度]
            s1 = size(S1);%S1の[行数,列数]
            ori_S1 = S1;
    ...
    if plot_fit
        if save_fit
            ...
            saveas(gcf,[pathname.fig,'/fit/',num2str(IDSP.date),'_shot', num2str(IDSP.shotlist(1)),'-',num2str(IDSP.shotlist(end)),'_',num2str(time),'us_fit_for_flow.png'])
            hold off
            close
        else
            hold off
    ...
    for k = 1:IDSP.n_r
        for i = 1:4
            i_CH = (k-1)*4+i;%CH番号
            S1 = [L1_shaped(:,i_CH)-offset(k,1) spectrum1_temp(:,i_CH)]; %[波長,強度]
            s1 = size(S1);%S1の[行数,列数]
            ori_S1 = S1;
            deleted_S1 = zeros(1,2);
