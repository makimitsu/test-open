PRO Photron_passive_Funato2022
...
fil=dialog_pickfile(path="\\192.168.1.111\experiment\results\Doppler\Photron\230103\pf39kV",filter="*.tif")
;restore,fil+"_accumulate.sav"
D=read_tiff(fil,image_index=0)
p=read_Ascii("\\192.168.1.111\experiment\results\Doppler\Photron\calibration_summary\2022_Funato_38CH\test2\radius.txt") & p=p.field1*1.e-3 & p=p
relative=read_Ascii("\\192.168.1.111\experiment\results\Doppler\Photron\calibration_summary\2022_Funato_38CH\test2\relative.txt") & relative=relative.field1
smile= read_Ascii("\\192.168.1.111\experiment\results\Doppler\Photron\calibration_summary\2022_Funato_38CH\test2\smile.txt") & smile=smile.field1
center= read_Ascii("\\192.168.1.111\experiment\results\Doppler\Photron\calibration_summary\2022_Funato_38CH\test2\center.txt") & center=center.field1
;center= read_Ascii("\\192.168.1.111\experiment\results\Doppler\Photron\calibration_summary\2022_sawagashira_38CH\center.txt") & center=center.field1 & center = reverse(center)
width=5
Instrument=read_ascii("\\192.168.1.111\experiment\results\Doppler\Photron\calibration_summary\2022_Funato_38CH\test2\instrument.txt") & Instrument=Instrument.field1
...
;   spectra[j,*,i]=spectra[j,*,i]-total(reform(spectra[j,*,*]),2)/(bg+1.); バックグラウンド信号除去
...
;for i=0,0 do begin
  for j=0,35 do begin
    fitting=gaussfit(reform(lambda2[*,j]),spectra[j,*,i],coeff,nterms=4,sigma=sigma)
...
spectra_interp=smooth(spectra_interp,[5,num/18.,2])

for i=0,t_end-t_start do begin
  for j=0,n_elements(lambda_out)-1 do begin
    dIdt=deriv(yy,reform(spectra_interp[j,*,i]))
    for k=0,num do begin
      Local_spectra[j,k,i]=total(-1./!PI*(dIdt/sqrt(yy^2-yy[k]^2)*dy),/nan,/double)
    endfor
  endfor
