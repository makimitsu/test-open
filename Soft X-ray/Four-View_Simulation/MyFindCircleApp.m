...
if isequal(fileNames, 0)
    ...
    if numel(fileNames) == 2
        ...
        % % 例えば、imreadを使用して画像を読み込む場合
        % img1 = imread(FiberCalibrationImagePath);
        % img2 = imread(ShotImagePath);
        % % ここに画像の処理や表示などを行う
    else
...
btn = uicontrol('Style', 'pushbutton', 'String', 'save data', ...
    'Position', [650, 30, 80, 30], 'Callback', {@save_calibration_data, OFFSET,CenterPositions,date});
...
function save_calibration_data(~,~,OFFSET,CenterPositions,date)
global OFFSET_fiber
for i=1:4
    for j=1:2
        OFFSET(i,:,j)=OFFSET_fiber(i,j);
