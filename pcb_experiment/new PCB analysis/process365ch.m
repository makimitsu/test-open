...
r_shift38 = 0.0; % 【input】プローブの差し込み具合を変更した場合は記入
r_shift39 = 0.00;
for i=1:n_data
    shot=shotlist(i,:);
    tfshot=tfshotlist(i,:);
    ...
    i_EF=EFlist(i);
    TF=TFlist(i);
    process_psi365ch(date,shot,tfshot,pathname,n,i_EF,trange,r_shift38,r_shift39,probecheck_mode,interp_method);
    disp('pcb:1/1')  
end
...
function process_psi365ch(date, shot, tfshot, pathname, n,i_EF,trange, r_shift38,r_shift39,probecheck_mode,interp_method)
%% read doi probe file
% 較正係数のバージョンを日付で判別
...
ok_bt_matrix_a39 = false(length(rprobepcb_a39),length(zprobepcb_a39));
...
grid2D=struct(...
    ...
    'ok_bt_matrix_a39',ok_bt_matrix_a39...
    );
...
filename = strcat("C:\Users\uswk0\OneDrive - The University of Tokyo\data\pre_processed",'\a03839_',num2str(shot(2)),'.mat');%保存ファイル名
save(filename)
end