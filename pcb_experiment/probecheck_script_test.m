...
%%%%%%%%%%%%%%%%%%%%%%%%
pathname.fig = ""; % plot画像の保存先

figure_switch = ["on","on","on","off"];%bz1,bz2,bz_vs_z, bz_vs_r

r = 5;%プローブ本数＝グラフ出力時の縦に並べる個数
col1 = 13;%グラフ出力時の横に並べる個数 1枚目
col2 = 12;%グラフ出力時の横に並べる個数 2枚目
r_ch = col1 + col2;% 2枚のグラフの合計（１本のプローブのch数）
y_upper_lim = 0.02;%縦軸プロット領域（b_z上限）
y_lower_lim = -0.02;%0.03;%縦軸プロット領域（b_z下限）
...
for probe_num = 0:(r-1)
    ...
    % zpos = 0[m]の位置のbzの値
    if probe_num < 3
        rline = [(1+probe_num*col):((probe_num+1)*col)];
    elseif probe_num == 3
        rline = [(1+probe_num*col):62,64:((probe_num+1)*col)];
    else
        rline = [(2+probe_num*col):((probe_num+1)*col+1)];
    end
    bz_rline=bz(t,rline);
    bz_rline(ok_bz(rline)==false)=NaN;
    plot(r_probe_pcb,bz_rline,'.','MarkerSize',12)
    %clear bz_rline r_line
    title(strcat('z = ',num2str(z_probe_pcb(i))));
