...
%直接入力の場合
dtacqlist=38;
shotlist=11435;%【input】dtacqの保存番号
tfshotlist=0;
...
C = readmatrix('/Users/yunhancai/Google Drive/磁気プローブ/matlab磁気プローブ解析new/pcb_experiment/coeff125ch.xlsx','Sheet',num2str(sheet_date));
ok = logical(C(:,14));
P=C(:,13);
coeff=C(:,12);
zpos=C(:,9);
...
p_ch= readmatrix('/Users/yunhancai/Google Drive/磁気プローブ/matlab磁気プローブ解析new/pcb_experiment/coeff125ch.xlsx','Sheet','p_ch');
...
ok_bz([5 54 31 8 61 37 62 13 38 16 40 20 45 47 23 48 25])=true;

%生信号描画用パラメータ
r = 5;%プローブ本数＝グラフ出力時の縦に並べる個数
col1 = 12;%1枚目のグラフ出力時の横に並べる個数
col2 = 13;%2枚目のグラフ出力時の横に並べる個数
y_upper_lim = 0.3;%3e-3;%0.1;%縦軸プロット領域（b_z上限）
y_lower_lim = -0.3;%3e-3;%-0.1;%縦軸プロット領域（b_z下限）
